...
class QuestionApiService {
  ...
  static Future<MCQ> updateMCQ(MCQ original, MCQ updated) async {
    ...
    if (original.question != updated.question) {
      changes['question'] = updated.question;
    }
    if (original.points != updated.points) changes['points'] = updated.points;
    if (!_listEquals(original.options, updated.options)) {
      changes['options'] = updated.options;
    }
    if (original.answerIndex != updated.answerIndex) {
      changes['answerIndex'] = updated.answerIndex;
    }
    if (original.bankId != updated.bankId) changes['bankId'] = updated.bankId;
    if (!_listEquals(original.variableIds, updated.variableIds)) {
      changes['variableIds'] = updated.variableIds;
    }
  ...
  static Future<void> deleteMCQ(String id) async {
    final response = await http.delete(Uri.parse('$baseUrl/mcqs/$id'));
  ...
  static Future<MSQ> updateMSQ(MSQ original, MSQ updated) async {
    ...
    if (!_listEquals(original.answerIndices, updated.answerIndices)) {
      changes['answerIndices'] = updated.answerIndices;
    }
    if (original.bankId != updated.bankId) changes['bankId'] = updated.bankId;
    if (!_listEquals(original.variableIds, updated.variableIds)) {
      changes['variableIds'] = updated.variableIds;
  ...
  static Future<void> deleteMSQ(String id) async {
    final response = await http.delete(Uri.parse('$baseUrl/msqs/$id'));
  ...
  static Future<void> deleteNAT(String id) async {
    final response = await http.delete(Uri.parse('$baseUrl/nats/$id'));
  ...
  static Future<Subjective> updateSubjective(
    Subjective original,
    Subjective updated,
  ) async {
    // Build PATCH payload with only changed fields
    ...
    if (original.points != updated.points) changes['points'] = updated.points;
    if (original.idealAnswer != updated.idealAnswer) {
      changes['idealAnswer'] = updated.idealAnswer;
    }
    if (!_listEquals(original.gradingCriteria, updated.gradingCriteria)) {
      changes['gradingCriteria'] = updated.gradingCriteria;
    }
    if (original.bankId != updated.bankId) changes['bankId'] = updated.bankId;
    if (!_listEquals(original.variableIds, updated.variableIds)) {
      changes['variableIds'] = updated.variableIds;
  ...
  static Future<void> deleteSubjective(String id) async {
    final response = await http.delete(Uri.parse('$baseUrl/subjectives/$id'));
