...
final class ReportCardGeneratorResponseMessage extends StatelessWidget implements AgentResponseMessage {
  final ReportCardAgentSerializer serializer;

  const ReportCardGeneratorResponseMessage({super.key, required this.serializer});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Assignments Completed: ${reportCard.overallPerformance.totalAssignmentsCompleted}',
              style: GoogleFonts.jost(fontSize: 14),
            ),
            Text(
              'Overall %: ${reportCard.overallPerformance.overallPercentage.toStringAsFixed(2)}',
              style: GoogleFonts.jost(fontSize: 14),
            ),
            Text('Trend: ${reportCard.overallPerformance.improvementTrend}', style: GoogleFonts.jost(fontSize: 14)),
            Text(
              'Strongest Q Type: ${reportCard.overallPerformance.strongestQuestionType}',
              style: GoogleFonts.jost(fontSize: 14),
            ),
            Text(
              'Weakest Q Type: ${reportCard.overallPerformance.weakestQuestionType}',
              style: GoogleFonts.jost(fontSize: 14),
            ),
            const Divider(height: 24),
            Text('Subjects', style: GoogleFonts.jost(fontWeight: FontWeight.w600, fontSize: 16)),
            ...reportCard.subjectPerformance.map(
              (subject) => Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Score: ${subject.percentageScore.toStringAsFixed(2)}%',
                      style: GoogleFonts.jost(fontSize: 13),
                    ),
                    Text('Assignments: ${subject.assignmentCount}', style: GoogleFonts.jost(fontSize: 13)),
                    Text(
                      'MCQ Accuracy: ${subject.mcqAccuracy.toStringAsFixed(2)}%',
                      style: GoogleFonts.jost(fontSize: 13),
                    ),
                    Text(
                      'MSQ Accuracy: ${subject.msqAccuracy.toStringAsFixed(2)}%',
                      style: GoogleFonts.jost(fontSize: 13),
                    ),
                    Text(
                      'NAT Accuracy: ${subject.natAccuracy.toStringAsFixed(2)}%',
                      style: GoogleFonts.jost(fontSize: 13),
                    ),
                    Text(
                      'Subjective Avg: ${subject.subjectiveAvgScore.toStringAsFixed(2)}',
                      style: GoogleFonts.jost(fontSize: 13),
                    ),
                    Text(
                      'Strengths: ${subject.strengths.join(', ')}',
                      style: GoogleFonts.jost(fontSize: 13, color: Colors.green[700]),
                    ),
                    Text(
                      'Weaknesses: ${subject.weaknesses.join(', ')}',
                      style: GoogleFonts.jost(fontSize: 13, color: Colors.red[700]),
                    ),
            ...
            ...reportCard.assignmentSummaries.map(
              (a) => Padding(
                padding: const EdgeInsets.symmetric(vertical: 2.0),
                child: Text(
                  '${a.assignmentTitle} (${a.subject}): ${a.percentageScore.toStringAsFixed(2)}%',
                  style: GoogleFonts.jost(fontSize: 13),
                ),
              ),
            ),
            ...
            Text(
              'Strengths: ${reportCard.studentInsights.keyStrengths.join(', ')}',
              style: GoogleFonts.jost(fontSize: 13, color: Colors.green[700]),
            ),
            Text(
              'Areas for Improvement: ${reportCard.studentInsights.areasForImprovement.join(', ')}',
              style: GoogleFonts.jost(fontSize: 13, color: Colors.red[700]),
            ),
            Text(
              'Recommended Actions: ${reportCard.studentInsights.recommendedActions.join(', ')}',
              style: GoogleFonts.jost(fontSize: 13),
            ),
          ],
        ),
